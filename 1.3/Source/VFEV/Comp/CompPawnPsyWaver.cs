    ...
    public class CompPawnPsyWaver : ThingComp
    {
        ...
        public override void CompTick()
        {
            ...
            if (this.parent is Pawn pawn && pawn.Spawned && Find.TickManager.TicksGame >= readyToUseTicks)
            {
                ...
                var victims = pawn.Map.mapPawns.AllPawns.Where(x => x.Position.DistanceTo(pawn.Position) < Props.maxDistance);
                FleckMaker.Static(pawn.Position, pawn.Map, FleckDefOf.PsycastAreaEffect, Props.maxDistance);
